...
>>foreach i in (A,C,D)
>>if ( -FILE DDCASPT2_e2_$i_1_.csv )
>>> COPY $WorkDir/DDCASPT2_RHS_$i_1_.csv $CurrDir/DDCASPT2_RHS_$i_1_.csv
>>> COPY $WorkDir/DDCASPT2_IVECW_$i_1_.csv $CurrDir/DDCASPT2_IVECW_$i_1_.csv
>>> COPY $WorkDir/DDCASPT2_IVECX_$i_1_.csv $CurrDir/DDCASPT2_IVECX_$i_1_.csv
>>> COPY $WorkDir/DDCASPT2_IVECC2_$i_1_.csv $CurrDir/DDCASPT2_IVECC2_$i_1_.csv
>>> COPY $WorkDir/DDCASPT2_e2_$i_1_.csv $CurrDir/DDCASPT2_e2_$i_1_.csv
>>endif
>>enddo
"""
        else:
...
"""
    ...
    def gen_pairs(self,i,root):
        ...
        typ=re.compile(r"DDCASPT2_e2_([A-Z](?:_[PM])?)\.csv").match(os.path.basename(i)).group(1)
        
        # labels = [ for f in filenames]
        # typ = os.path.basename(i).split('.')[0].replace('DDCASPT2_e2_','')#.replace(f'_','')
    ...
    def gen_feats(self):
        ...
        for msr in tqdm(msroots_range,desc="Root"):
            if self.n_jobs is None:
                self.pairs = np.vstack([self.gen_pairs(i,msr) for i in tqdm(glob(os.path.join(self.path,f"DDCASPT2_e2_*.csv")),desc="Pairs")])    
            else:
                self.pairs = np.vstack(Parallel(n_jobs=self.n_jobs)(delayed(self.gen_pairs)(i,msr) for i in tqdm(glob(os.path.join(self.path,f"DDCASPT2_e2_*.csv")),desc="Pairs")))  
