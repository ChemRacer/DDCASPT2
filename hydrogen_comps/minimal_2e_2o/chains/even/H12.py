...
h5list = sorted(glob('H12_chain/*/*rasscf.h5'))
...
h5list_scf = sorted(glob('H12_chain/*/*.scf.h5'))
...
typ_exists=sorted(sum(list([j.replace('GMJ_e2_','') for j in i.split('/')[-1].split('.') if 'GMJ' in j] for i in glob('H12_chain/H12_0.60/GMJ_e2_*.csv')),[]))
...
dims_dict=gen_dim_dict('H12_chain/H12_0.60/',typ_exists)
...
E2Dict=pd.read_csv(f"H12_chain/E2.csv",index_col=0).to_numpy()
...
path_check='H12_chain/H12_0.60/H12_0.60.output'
...
def Big_Data_GS():
    t0=time()
    with open('H12_chain/fixed_feats.pickle', 'wb') as handle:
    ...
    with open('H12_chain/fixed_targets.pickle', 'wb') as handle:
